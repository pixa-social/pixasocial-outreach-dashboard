...
(async () => {
  const [t0, t1, b2, b3] = await Promise.all([
    ...
    fetch('dashboard.p2.b64.txt', { cache: 'no-store' }).then((r) => { if (!r.ok) throw new Error(r.status); return r.text(); }),
    fetch('dashboard.p3.b64.txt', { cache: 'no-store' }).then((r) => { if (!r.ok) throw new Error(r.status); return r.text(); }),
  ]);
